...
fs.readFileSync('python-3d-rendering.md.literate', {encoding: 'utf8'}).split('\n').forEach(function(line) {
	...
	if (STATE_NORMAL == state) {
		if (startsWith(line, '```python')) {
			...
			post.write("<div class='well'>This post was written in literate Python. Download [buildshape.py](buildshape.py) and run it yourself!</div>\n\n")
		...
		if (startsWith(line, '```')) {
			if (!codeblock_hidden) {
				// The {% raw %} is needed to keep the code from being interpreted as markdown
				post.write('\n{% raw %}'+
		...
		if (startsWith(line, '\\end{tikzpicture}')) {
			...
			post.write('\n{% raw %}<figure style="float:right;margin:30px 0"><img src="'+file+'-1.png" alt="" srcset="'+file+'-2x-1.png 2x, '+file+'-3x-1.png 3x"/></figure>{% endraw %}\n')
